...
from fitness_tracker.notes.workouts.workouts_db import db_info
...
def create_user_test_table(path):
  ...
  create_user_info_after_signup(test_user, test_user["email"], test=[True, "".join([test_user["email"], "_table"])], path=path)
